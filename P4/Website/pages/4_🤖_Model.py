...
model = get_model('C:\\Users\\elian\\website\\data\\abr\\serialized_model.json')


@st.cache(allow_output_mutation=True)
def get_master():
    return pd.read_csv(r"C:\Users\elian\website\data\Master.CSV", usecols=['CRASH DATE', 'CRASH TIME', 'BOROUGH'])
...
@st.cache(allow_output_mutation=True)
def get_mldf():
    return pd.read_csv(r"C:\Users\elian\website\data\abr\mldf_daily.CSV")
...
@st.cache(allow_output_mutation=True)
def get_df_4plot():
    return pd.read_csv(r"C:\Users\elian\website\data\abr\df_4plot_daily.CSV")
...
def get_mldf_monthly():
    return pd.read_csv(r"C:\Users\elian\website\data\abr\mldf_monthly.CSV")
...
def get_monthly_df_4plot():
    return pd.read_csv(r"C:\Users\elian\website\data\abr\df_4plot_monthly.CSV")
...
for bor in ['bronx', 'brooklyn', 'manhattan', 'queens', 'staten']:
    for name in ['_4plot_daily', '_4plot_monthly', '_mldf_daily', "_mldf_monthly"]:
        ...
        p = 'C:\\Users\\elian\\website\\data\\abr\\' + bor + name + '.CSV'
        exec(
            bor + name + "= get_bor(p)")  # exec is used as a way to set variables in which the var names come from strings. Here: bor and name are actually strings.
...
time_form = st.date_input("Specify Date:", datetime.date(2022, 1, 1), min_value=datetime.date(2012, 7, 1),
                          max_value=datetime.date(2023, 12, 21))
...
if time_form:
    ...
    with left1:
        if time_form >= df_4plot['ds'].min():
            st.markdown(
                '<p style="font-family:sans-serif; color:rgb(231,107,243); font-size: 14px; text-align: left">{}</p>'.format(
                    'Expected Number of Collisions on ' + str(time_form)[:-9] + ':'),
                unsafe_allow_html=True)
            st.markdown(
                '<p style="font-family:sans-serif; color:rgb(231,107,243); font-size: 30px; text-align: left">{}</p>'.format(
                    str(df_4plot[df_4plot['ds'] == time_form]['y'].iloc[0])[:-2]),
                unsafe_allow_html=True)
            st.markdown(
                '<p style="font-family:sans-serif; color:rgb(231,107,243); font-size: 14px; text-align: left">Uncertainty Interval: ({lower}, {upper})</p>'.format(
            ...
            if time_form >= mldf['ds'].min():
                st.markdown(
                    '<p style="font-family:sans-serif; color:rgb(0,176,246); font-size: 14px; text-align: left">{}</p>'.format(
                        'Number of Collisions on ' + str(time_form)[:-9] + ':'),
                    unsafe_allow_html=True)
                st.markdown(
                    '<p style="font-family:sans-serif; color:rgb(0,176,246); font-size: 30px; text-align: left">{}</p>'.format(
    ...
    with right1:
        if time_form >= df_4plot['ds'].min():
            st.markdown(
                '<p style="font-family:sans-serif; color:rgb(231,107,243); font-size: 14px; text-align: center">{}</p>'.format(
                    'Predicted Percentage of Collisions by Borough'),
                unsafe_allow_html=True)
            ...
            st.markdown(
                '<p style="font-family:sans-serif; color:rgb(0,176,246); font-size: 14px; text-align: center">{}</p>'.format(
                    'Percentage of Collisions by Borough'),
                unsafe_allow_html=True)
...
if time_selected == "Daily":
    ...
    fig.add_trace(
        go.Scatter(x=list(df_4plot['ds']) + x_rev, y=list(df_4plot['yhat_upper']) + list(df_4plot['yhat_lower'][::-1]),
                   fill='toself',
    ...
    fig.update_layout(
        xaxis_range=[mldf.iloc[-1, 0] - relativedelta(weeks=15), mldf.iloc[-1, 0] + relativedelta(weeks=3)])
    ...
    fig.add_trace(go.Scatter(x=list(monthly_df_4plot['ds']) + x_rev,
                             y=list(monthly_df_4plot['yhat_upper']) + list(monthly_df_4plot['yhat_lower'][::-1]),
                             fill='toself',
                             fillcolor='rgba(231,107,243,0.2)',
                             line_color='rgba(255,255,255,0)',
                             mode='lines',
...
with mid3:
    if patt_selected == "Day of the Week":
        st.write(
            "The first plot is for when school is in session. The second plot is for when school is not in session.")
